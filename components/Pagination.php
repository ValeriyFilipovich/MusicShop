...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<ul class="pagination pagination-lg justify-content-end">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<li class="page-item disabled"><a href="#" class="page-link active rounded-0 mr-3 shadow-sm border-top-0 border-left-0">' . $page . '</a></li>';
            } else {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        return
                '<li><a class="page-link rounded-0 mr-3 shadow-sm border-top-0 border-left-0 text-dark" href="' . $currentURI . $this->index . $page . '">' . $text . '</a></li>';
    }
